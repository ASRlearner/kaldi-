...
//运用在线倒谱均值(可能还有方差)在线计算，使用与online2/和online2bin/下新的配置使用的相同代码
//如果使用--spk2utt选项，语音的开始阶段将使用来自同一个说话人先前的语音来回退
int main(int argc, char *argv[]) {
  try {
    ...
    po.Read(argc, argv);
    //如果参数个数不为3 则输出用法并报错退出
    if (po.NumArgs() != 3) {
      po.PrintUsage();
      exit(1);
    }
    //global_stats_rxfilename存放第一个参数值，也就是cmvn.scp内相关的cmvn矩阵存放地址
    //feature_rspecifier读取第二个参数，实际为feats.scp中特征存放的地址
    //feature_wspecifier写入，在样例中将特征写入标准输出
    std::string global_stats_rxfilename = po.GetArg(1),
        feature_rspecifier = po.GetArg(2),
        feature_wspecifier = po.GetArg(3);


    //global_cmvn_stats 在语音的开始阶段帮助我们实现初始化在线cmvn成合理的值
    //它实际为一个矩阵
