...
bool GetLinearSymbolSequence(const Fst<Arc> &fst,
  ...
  if (cur_state == kNoStateId) {  // 如果是空序列
  ...
  while (1) {
    ...
    if (w != Weight::Zero()) {  // 最终的情况
      //将权重(包含两个浮点权值 图(语言模型)代价和声学代价)的值对应相加得到总权重
      tot_weight = Times(w, tot_weight);
      //如果该状态id对应的弧个数不为0 则返回false
